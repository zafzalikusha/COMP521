...
% 
% for O = 1:5
%     max_errors = cellfun(@(x) max(x), errors_first_derivative(:, O));
%     loglog(h_values, max_errors, '-o', 'DisplayName', ['Order ', num2str(O)]);
%     hold on;
% 
%     % Fit a line to the log-log plot
%     p1 = polyfit(log(h_values), log(max_errors), 1);
% 
...
% subplot(2, 1, 2);
% for O = 1:5
%     max_errors = cellfun(@(x) max(x), errors_second_derivative(:, O));
%     loglog(h_values, max_errors, '-o', 'DisplayName', ['Order ', num2str(O)]);
%     hold on;
% 
%     % Fit a line to the log-log plot
%     p2 = polyfit(log(h_values), log(max_errors), 1);
% 
%     % Store the slope in the array
...
% subplot(2, 1, 1);
% for O = 1:5
%     plot(h_values, cell2mat(times_first_derivative(:, O)), '-o', 'DisplayName', ['Order ', num2str(O)]);
%     hold on;
% end
% title('Time for First Derivative');
% xlabel('Step Size (h)');
% ylabel('Time (seconds)');
% legend('Location', 'Best');
% grid on;
...
% subplot(2, 1, 2);
% for O = 1:5
%     plot(h_values, cell2mat(times_second_derivative(:, O)), '-o', 'DisplayName', ['Order ', num2str(O)]);
%     hold on;
% end
% title('Time for Second Derivative');
% xlabel('Step Size (h)');
% ylabel('Time (seconds)');
% legend('Location', 'Best');
% grid on;
