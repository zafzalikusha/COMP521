...
for i = 1:length(grid_sizes)
    ...
    % Calculate the error
    errors(i) = max(abs(f_double_prime_exact(2:end - 1) - f_double_prime_approx(2:end - 1)));
...
figure;
plot(x_values{h_index}, numerical_results{h_index}, 'm--', 'LineWidth', 2, 'DisplayName', 'Numerical (h = 0.02)');
hold on;
plot(x_values{h_index}, analytical_results{h_index}, 'co', 'LineWidth', 2, 'DisplayName', 'Analytical (h = 0.02)');
xlabel('x');
ylabel("f''(x)");
